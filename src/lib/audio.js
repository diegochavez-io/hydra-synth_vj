...
class Audio {
  constructor ({
    ...
    window.navigator.mediaDevices.getUserMedia({
      ...
      .then((stream) => {
        ...
        // after permission is granted, check if we landed on a virtual device
  ...
  _initFromStream (stream) {
    ...
    // disconnect previous source if switching
  ...
  _preferPhysicalMic () {
    ...
    window.navigator.mediaDevices.enumerateDevices().then((devices) => {
      ...
      if (!virtualPatterns.test(currentLabel)) return

      // find a physical mic — prefer "macbook" or "built-in", else first non-virtual
      var physicalMic = audioInputs.find((d) =>
        /macbook|built.in/i.test(d.label)
      ) || audioInputs.find((d) =>
        !virtualPatterns.test(d.label) && d.label !== 'Default'
      )
